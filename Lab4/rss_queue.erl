...
% Таймаут на время ожидания приема сообщений.
-define(TIMEOUT,1000).
...
%      QPid - это PID процесса очереди, Item - это элемент, добавляемый к очереди.
...
% @doc Функция извлекает все элементы из документа ленты и оправляет их по порядку в очередь.
...
% @doc Вспомогательная функция, упрощает процедуру получения списка элементов ленты от процесса.
...
% @private
% @doc Вспомогательная функция, инициирующая процесс добавления нового элемента ленты в очередь.
...
% @private
% @doc Вспомогательная функция, осуществляющая добавление нового элемента в пустую очередь.
...
% @private
% @doc Вспомогательная функция, осуществляющая добавление нового элемента в непустую очередь по правилу:
%      если same - новый элемент отбрасывается,
%      если updated - старая запись удаляется, а новая записывается соответственно дате,
%      если different - новый элемент добавляем в порядке возрастания даты.
...
% @private
% @doc Функция сортировки списка элементов очереди по возрастанию даты публикации.
