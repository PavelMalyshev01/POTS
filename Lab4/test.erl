-module(test).
-export([testing/1, testing/2, print_list/1]).
...
% @doc Загрузка в очередь XML файла.
% @spec testing(RSSFile::filename()) -> ok | {unknown_msg | error, string() | timeout}
%
testing(RSSFile) ->
	PID = rss_queue:start(),
...
% @doc Загрузка двух файлов с различными временами
% @spec testing(RSSFile1::filename(),RSSFile2::filename()) -> ok | {unknown_msg | error, string() | timeout}
%
testing(RSSFile1, RSSFile2) ->
	PID = rss_queue:start(),
